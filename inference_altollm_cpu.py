...
# Configurar para usar exclusivamente CPU se necessário
import os
...
# Forçar uso de CPU
print("Iniciando carregamento do modelo (CPU only)...")
path = 'yayafengzi/ALToLLM-8B'

# Carregamento simples em CPU
...
generation_config = dict(max_new_tokens=256, do_sample=False, return_ids=True)  # Reduzir mais ainda

# Processar apenas uma imagem por vez 
image_path = './imgs/image1.jpg'
print(f"Carregando imagem: {image_path}")
image = Image.open(image_path).convert('RGB')
pixel_values = load_image(image, max_num=1).to(torch.float32)  # CPU usa float32

num_patches_list = [pixel_values.size(0)]

question = '<image>\nSegment <ref>animal</ref> by adaptive length.'
print("Iniciando inferência (pode demorar em CPU)...")
...
try:
    ...
    # Processamento básico do resultado sem máscara completa por enquanto
    print("Inferência concluída com sucesso em CPU!")

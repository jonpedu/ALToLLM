...
from net.modules.base_model import BaseModel
from net.modules.blocks_multi_length import TiTokEncoder,TiTokDecoder
from net.quantizer import VectorQuantizer
from net.modules.maskgit_vqgan import Encoder as Pixel_Eecoder
from net.modules.maskgit_vqgan import Decoder as Pixel_Decoder
from net.modules.maskgit_vqgan import VectorQuantizer as Pixel_Quantizer
import json
from omegaconf import OmegaConf
from pathlib import Path
from net.modules.segment_anything.build_sam import sam_model_registry
import numpy as np
import os
# from diffusers import AutoencoderKL
import random
class ALTo(BaseModel):
    def __init__(self, config):
        super().__init__()
        if isinstance(config, dict):
            config = OmegaConf.create(config)
        self.config = config
        self.select_vae_info_inject = False
        ### encoder
        ...
        ### vae
        # vae="/mnt/wlf/models/mar-vae-kl16"
        ...
        ## sam decoder
        real_sam_checkpoint = config.experiment.sam_checkpoint if os.path.exists(config.experiment.sam_checkpoint) else None
        if 'sam_vit_h' in config.experiment.sam_checkpoint:
            self.sam = sam_model_registry['vit_h'](checkpoint=real_sam_checkpoint)
        elif 'sam_vit_b' in config.experiment.sam_checkpoint:
            self.sam = sam_model_registry['vit_b'](checkpoint=real_sam_checkpoint)
        elif 'sam_vit_l' in config.experiment.sam_checkpoint:
            self.sam = sam_model_registry['vit_l'](checkpoint=real_sam_checkpoint)
        else:
    ...
    def encode(self, x):
        z, length_probs, length_loss, lengths_to_keep = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens)
        z_quantized, result_dict = self.quantize(z)
        z_quantized = z_quantized * length_probs.unsqueeze(1).unsqueeze(1) # N,D,1,L * N,1,1,L -> N,D,1,L
        result_dict.update({"length_loss":length_loss, "lengths_to_keep":lengths_to_keep})
    ...
    def decode_tokens(self, tokens, image_src=None):
        if self.quantize_mode == "vq":
            ...
            z_quantized = tokens
        decoded = self.decode(z_quantized, image_src=image_src)[0]
        return decoded
    ...
    def decode_token_only(self, z_quantized, image_src=None):
        ...
        # decoded_image = decoded.mean(1, keepdim=True)#.repeat(1, 3, 1, 1)  #sigmoid(x)

        masks_logits = (decoded.mean(1, keepdim=True) * 2 - 1) * 6
        decoded_image = masks_logits.sigmoid()

        # extra_result_dict["masks_logits"] = decoded_image
    ...
    def decode_token_by_vae(self, z_quantized, image_src, lengths_to_keep=None):
        # image_src = F.interpolate(image_src, (256, 256), mode='bilinear', align_corners=False)
        # image_embedding = self.vae_encode(image_src)  #torch.Size([b, 16, 64, 64])
        image_src = image_src*255   #b,3,1024,1024
        image_src = self.sam.preprocess(image_src)  
        image_embedding = self.sam.image_encoder(image_src)
        ...
        decoded, lengths_to_keep = self.decoder(z_quantized,    # torch.Size([b, 12, 1, 32])
                                                ...
                                                test_length=self.config.model.test_length,
                                                tune_decoder=self.config.model.vq_model.finetune_decoder,
    ...
    def decode_sam(self, z_quantized, image_src=None):
    ...
    @torch.no_grad()
    def vae_decode(self, z):# z:torch.Size([1, 16, 16, 16])
        z = self.vae.decode(z / self.scaling_factor)# process z:torch.Size([1, 3, 256, 256])
        if hasattr(z, 'sample'):
            z = z.sample
        return (z + 1) / 2 
    ...
    def forward(self, x, image_src=None):
        ...
        #     self.select_vae_info_inject = False
        # else:
        #     self.select_vae_info_inject = True
        
        # if self.select_vae_info_inject:
        if self.config.model.use_random_morphology_augment:
        ...
        decoded_image, extra_result_dict = self.decode_token_by_vae(z_quantized, image_src, lengths_to_keep=encode_extra_result.get("lengths_to_keep", None))
        # decoded_image, extra_result_dict = self.decode_token_only(z_quantized)
        encode_extra_result.update(extra_result_dict)
        encode_extra_result.update({"random_morphology_augment":x})

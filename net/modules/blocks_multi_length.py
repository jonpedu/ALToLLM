...
class LengthEncoder(nn.Module):
    def __init__(self, width,use_random_not_adaptive=False):
        super().__init__()
        self.width = width
        ...
        self.use_random_not_adaptive = use_random_not_adaptive
    def forward(self, x, q):  # x:         
        L=x.shape[1]
        ...
        one_hot = torch.zeros_like(attn_probs)  # [N, L]
        one_hot.scatter_(-1, hard_probs.unsqueeze(-1), 1)  # [N, L]
        # Straight-through estimator for backpropagation through argmax
        probs = one_hot + attn_probs - attn_probs.detach()  # [N, L]

        if self.use_random_not_adaptive:
            probs = random_onehot_with_distribution(x.shape[0], L, device=x.device)

        length_to_keep = torch.argmax(probs, dim=-1)+1

        probs = torch.cat([torch.zeros_like(probs[:,:1]), probs[:,:-1].cumsum(dim=-1)], dim=1)  # [N, L] 累积前n-1个概率
        probs = 1 - probs 

        return probs, length_loss, length_to_keep
...
class TiTokEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.length_encoder = LengthEncoder(self.width,use_random_not_adaptive=config.model.get("use_random_not_adaptive", False))
    ...
    def forward(self, pixel_values, latent_tokens):
        ...
        length_probs,length_loss,length_to_keep = self.length_encoder(latent_tokens, cls_token)
        # fake 2D shape
        if self.is_legacy:
        ...
        return latent_tokens, length_probs, length_loss, length_to_keep
...
class TiTokDecoder(nn.Module):
    ...
    def random_mask_tail_tokens(self, x, gamma=4, bs_no_image=None, test_length=32):
        ...
        # lengths_to_keep[:bs_no_image] = torch.clamp(lengths_to_keep[:bs_no_image], min=15)
    ...
    def forward(self, z_quantized, pixel_decoder=None, **kwargs):
        random_length = kwargs.get("random_length", True)
        test_length = kwargs.get("test_length", 16)
        image_embedding = kwargs.get("image_embedding", None) 
        use_vae = kwargs.get("use_vae", True)
        select_vae_info_inject = kwargs.get("select_vae_info_inject", True)
        tune_decoder = kwargs.get("tune_decoder", False)
        lengths_to_keep = kwargs.get("lengths_to_keep", None)
        ...
        if image_tokens is not None:
            ...
            if use_vae:
                # image_tokens[bs_no_image//2:bs_no_image//2+bs_no_image,:,:] = 0
                # image_tokens[bs_no_image//2+bs_no_image:,:,:] = 0
                # lengths_to_keep[:bs_no_image//2+bs_no_image] += 100
                # image_tokens[:2,:,:] = 0
                # lengths_to_keep[2:] += 100
...
def randomly_shift_onehot(one_hot_tensor, max_shift=None):
    """
    Randomly shifts the position of 1s in a one-hot tensor to later positions.
    
    Args:
        one_hot_tensor: A tensor of shape [N, L] with one-hot encoding
        max_shift: Maximum number of positions to shift (default: up to end of sequence)
    
    Returns:
        Shifted one-hot tensor of same shape [N, L]
    """
    N, L = one_hot_tensor.shape
    
    # Find the current positions of 1s
    indices = torch.argmax(one_hot_tensor, dim=1)  # [N]
    
    # Determine maximum possible shift for each sample
    if max_shift is None:
        max_possible_shift = L - 1 - indices  # How far each 1 can be shifted
    else:
        max_possible_shift = torch.minimum(max_shift * torch.ones_like(indices), L - 1 - indices)
    
    # Generate random shifts (between 1 and max_possible_shift)
    # We add 1 because we want at least 1 position of shift
    random_shifts = torch.zeros_like(indices)
    for i in range(N):
        if max_possible_shift[i] > 0:
            random_shifts[i] = torch.randint(1, max_possible_shift[i] + 1, (1,), device=one_hot_tensor.device).item()
    
    # Calculate new positions
    new_indices = indices + random_shifts
    
    # Create new one-hot tensor
    new_one_hot = torch.zeros_like(one_hot_tensor)
    for i in range(N):
        if new_indices[i] < L:  # Safety check
            new_one_hot[i, new_indices[i]] = 1
    
    return new_one_hot

def random_onehot_with_distribution(batch_size, length, probabilities=None, device=None):
    """
    基于给定概率分布生成随机的 one-hot 向量
    
    Args:
        batch_size: 批量大小
        length: one-hot 向量的长度
        probabilities: 形状为 [length] 的概率分布，如果为 None 则使用均匀分布
        device: 张量所在设备
    """
    if probabilities is None:
        probabilities = torch.ones(length, device=device)
    else:
    ...
    # 创建全零张量
    one_hot = torch.zeros(batch_size, length, device=device)

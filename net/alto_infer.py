...
from net.modules.blocks_multi_length_infer import TiTokEncoder,TiTokDecoder
...
class ALTo(BaseModel):
    ...
    def encode(self, x):
        z, length_indices = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens)
        z_quantized, result_dict = self.quantize(z)
        result_dict.update({"length_indices":length_indices})
    ...
    def decode_token_by_vae(self, z_quantized, image_src,image_embedding=None):
    ...
    def forward(self, x, image_src=None):
        ...
        decoded_image, extra_result_dict = self.decode_token_by_vae(z_quantized, image_src)

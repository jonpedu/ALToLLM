...
from internvl.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      ...
                                      REF_START_TOKEN, SEG_END_TOKEN,
                                      SEG_START_TOKEN, SEG_TOKEN_TEMPLATE,
                                      COODBOOK_SIZE)
...
from internvl.train.trainer_grpo import SegGRPOTrainer
...
class ModelArguments:
    ...
    decoder_weights: str = field(
        default=None,
        metadata={'help': 'Ckpt path to load the decoder weights.'}
    )
    freeze_decoder: bool = field(
        default=True,
        metadata={'help': 'Set to True to freeze the decoder.'}
    )
    num_token_trained: int = field(
        default=0,
        metadata={'help': 'Set the number of tokens to train.'}
    )
    mask_loss_weight: float = field(
        default=0,
        metadata={'help': 'Set the weight of the mask loss.'}
    )
    cos2fine: int = field(
        default=0,
        metadata={'help': 'How many levels of hirarchical masks to train.'}
    )
...
class DataTrainingArguments:
    ...
    length_weight: float = field(
        default=0.0,
        metadata={'help': 'Set the desired length weight for the image. Default is 0.5.'},
    )
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            try:
                ...
                if self.train_mask:
                    if "mask" in data_item:
                        mask = os.path.join(self.root, data_item["mask"])
                        mask = Image.open(mask).convert('L').resize((256,256))
                        mask = np.array(mask) / 255.0
                        ret['target_masks'] = torch.from_numpy(mask)
                    else:
                        ret['target_masks'] = torch.zeros((256,256))
                break
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            random_seed=ds_idx,
            train_mask=model.mask_decoder.num_token_trained > 0,
        )
...
def setup_mask_decoder(model, tokenizer, decoder_weights=None, freeze_decoder=True, mask_loss_weight=0, num_token_trained=0, cos2fine=0):
    model.mask_loss_weight = mask_loss_weight
    if model.mask_decoder is not None:  
        if decoder_weights is not None:
            model.mask_decoder.load_weights_from_ckpt(decoder_weights)
        model.mask_decoder.init_tt_ids(tokenizer)
        model.mask_decoder.cos2fine = cos2fine
        model.mask_decoder.num_token_trained = num_token_trained
        model.mask_decoder.set_requires_grad(not freeze_decoder)
        model.mask_decoder.set_encoder_requires_grad(False)
...
def main():
    ...
    token_list = [IMG_START_TOKEN, IMG_END_TOKEN, IMG_CONTEXT_TOKEN,
                  ...
                  SEG_START_TOKEN, SEG_END_TOKEN]
    token_list += [SEG_TOKEN_TEMPLATE.format(i) for i in range(COODBOOK_SIZE)]
    ...
    if model_args.model_name_or_path is not None:
        ...
        print(f"config from {model_args.model_name_or_path}: {config}")
        ref_model = ALToLLM.from_pretrained(
            model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config)
    ...
    setup_mask_decoder(model, tokenizer, 
                       decoder_weights=model_args.decoder_weights, 
                       freeze_decoder=model_args.freeze_decoder, 
                       mask_loss_weight=model_args.mask_loss_weight, 
                       num_token_trained=model_args.num_token_trained,
                       cos2fine=model_args.cos2fine)

    setup_mask_decoder(ref_model, tokenizer, 
                       decoder_weights=model_args.decoder_weights, 
                       freeze_decoder=model_args.freeze_decoder, 
                       mask_loss_weight=model_args.mask_loss_weight, 
                       num_token_trained=model_args.num_token_trained,
    ...
    assert model.config.vision_config.image_size == data_args.force_image_size
    ...
    if num_new_tokens > 0:
        ...
        ref_model.language_model.resize_token_embeddings(len(tokenizer))
        ref_model.config.llm_config.vocab_size = len(tokenizer)
        ref_model.language_model.config.vocab_size = len(tokenizer)

    model.language_model.config.use_cache = False
    model.vision_model.gradient_checkpointing = True
    model.vision_model.encoder.gradient_checkpointing = True
    ref_model.language_model.config.use_cache = False
    ref_model.vision_model.gradient_checkpointing = True
    ref_model.vision_model.encoder.gradient_checkpointing = True
    if model_args.grad_checkpoint:
        model.language_model._set_gradient_checkpointing()
        ref_model.language_model._set_gradient_checkpointing()
    ...
    trainer = SegGRPOTrainer(
        ...
        length_weight=data_args.length_weight,
    )
